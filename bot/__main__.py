...
from .func import code, cover_dl, run_async, stats
from .rename import _rename, get_cover, get_poster
        ...
        await event.reply(
            ...
            buttons=[
                [
                    ...
                    Button.url("Ask Repo", url="https://telegram.me/dev_shadow"),
                ]
            ],
        )
    await xnx.delete()
    ...
    try:
        ...
        if os.path.exists(dl):
            if compress:
                ...
                cmd = f'''{Var.FFMPEG} -i """{dl}""" -metadata "Encoded By"="t.me/Anime_Vertix" -preset ultrafast -c:v libx265 -crf 25 -map 0:v -c:a aac -map 0:a -c:s copy -map 0:s? """{out}""" -y'''
